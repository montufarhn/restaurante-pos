...
const CapacitorSQLite = window.Capacitor?.Plugins?.CapacitorSQLite || window.CapacitorSQLite;
const useCapacitorSqlite = isCapacitorPlatform && !!CapacitorSQLite;

let capacitorConn = null;

async function initCapacitorSQLite() {
    if (!useCapacitorSqlite) {
        console.log('LocalDB: Capacitor SQLite no detectado, usando localStorage');
        return;
    }

    try {
        const SQLiteConnection = window.SQLiteConnection || CapacitorSQLite?.SQLiteConnection;
        if (!SQLiteConnection && !window.CapacitorSQLite) {
            console.warn('LocalDB: SQLiteConnection no disponible');
            return;
        }

        // Si es Capacitor 5+, la conexión se crea diferente o a través del plugin directamente
        const sqlite = window.CapacitorSQLite || CapacitorSQLite;

        if (sqlite && typeof sqlite.createConnection === 'function') {
            capacitorConn = await sqlite.createConnection('restaurantepos', false, 'no-encryption', 1);
            await capacitorConn.open();
        } else {
             console.warn('LocalDB: Plugin SQLite no tiene createConnection');
             return;
        }
...
const LocalDB = (function () {
    ...
    function ensureInitialized() {
        ...
        // Verificación de integridad: asegurar que campos vitales existen
        const def = defaults();
        Object.keys(def).forEach(key => {
            if (!data[key]) data[key] = def[key];
        });

        // Asegurar que haya al menos un admin
        if (!data.usuarios || data.usuarios.length === 0) {
            data.usuarios = def.usuarios;
        }
    ...
    return {
        ...
        login: async (username, password) => {
            ...
            // Intento con SQLite si está disponible
            if (isCapacitorMode && capacitorConn) {
                try {
                    const result = await capacitorConn.query({
                        statement: 'SELECT id, username, role FROM usuarios WHERE username=? AND password=?',
                        values: [username, password]
                    });
                    if (result.values && result.values.length > 0) {
                        const user = result.values[0];
                        const session = { id: user.id, username: user.username, role: user.role };
                        localStorage.setItem('restaurante_pos_session', JSON.stringify(session));
                        return session;
                    }
                } catch (e) {
                    console.error('Error login SQLite:', e);
                }
            }

            // Fallback a memoria/localStorage
        ...
        getUsers: async () => {
            ensureInitialized();
            if (isCapacitorMode && capacitorConn) {
                const rows = await capQuery('SELECT id, username, role FROM usuarios', []);
                return rows || [];
            }
            return data.usuarios.map(u => ({ id: u.id, username: u.username, role: u.role }));
        },
        createUser: async (user) => {
            ensureInitialized();
            if (isCapacitorMode && capacitorConn) {
                await capExecute(`INSERT INTO usuarios (username, password, role) VALUES ('${user.username.replace("'","''")}', '${user.password.replace("'","''")}', '${user.role}')`);
                const rows = await capQuery('SELECT id, username, role FROM usuarios ORDER BY id DESC LIMIT 1', []);
                return rows[0] || null;
            }
            const newUser = { id: nextId(data.usuarios), ...user };
        ...
        updateUser: async (id, fields) => {
            ensureInitialized();
            if (isCapacitorMode && capacitorConn) {
                const updates = Object.entries(fields).map(([k,v]) => `${k}='${v.toString().replace("'","''")}'`).join(', ');
                await capExecute(`UPDATE usuarios SET ${updates} WHERE id=${id}`);
                const rows = await capQuery('SELECT id, username, role FROM usuarios WHERE id=?', [id]);
                return rows[0];
            }
            const index = data.usuarios.findIndex(u => u.id == id);
            if (index < 0) throw new Error('Usuario no encontrado');
            data.usuarios[index] = { ...data.usuarios[index], ...fields };
            save();
            const { password, ...safeUser } = data.usuarios[index];
            return safeUser;
        },
        updateUserPassword: async (id, newPassword) => {
            ensureInitialized();
            if (isCapacitorMode && capacitorConn) {
                await capExecute(`UPDATE usuarios SET password='${newPassword.replace("'","''")}' WHERE id=${id}`);
                return true;
            }
            const index = data.usuarios.findIndex(u => u.id == id);
            if (index < 0) throw new Error('Usuario no encontrado');
        ...
        deleteUser: async (id) => {
            ensureInitialized();
            if (isCapacitorMode && capacitorConn) {
                await capExecute(`DELETE FROM usuarios WHERE id=${id}`);
                return true;
            }
            const index = data.usuarios.findIndex(u => u.id == id);
            if (index < 0) throw new Error('Usuario no encontrado');
